...
###########################
    ...
    b_condition =  ema_trend_1m >= -3 and ema_trend_1m <= 1 and sum_score_trend_rest >= 3
    ...
    short_s = ema_trend_1m <= 3 and ema_trend_1m >= -1  and sum_score_trend_rest <= -3
...
########################################
    if st.session_state.stop_sleep == 0:
        ...
        st.write(f"B: {b_condition}__SS: {short_s}__S: {s_condition}__SB:{short_b}__Status_0: {SB == "AAA" or SB == "S" or SB == "SB"}__interval: {interval}")
       # st.empty()
        st.rerun()
